    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            if is_gad_mode:
                ...
                result = {"reward_tensor": reward_tensor, "reward_extra_info": {}}
            else:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if is_gad_mode:
                            ...
                            reward_extra_infos_dict = {}
                            gad_values = self._compute_values(batch)
                            batch = batch.union(gad_values)
                            reward_tensor = batch.batch["values"]
                        else:
                            # Standard mode: compute reward model score
                            if self.use_rm and "rm_scores" not in batch.batch.keys():
                                if not self.use_reward_loop:
                    ...
                    # compute values (skip in GAD mode as values already computed for reward)
                    if self.use_critic and "values" not in batch.batch:
